...
data class DamageReport(
    val hit: ArrayList<Position>,
    val miss: ArrayList<Position>,
    val sunk: ArrayList<ArrayList<Position>>
)
        ...
        when (val result = grid.takeFire(x, y)) {
            is Result.Success -> {
                when (val cellState = result.value) {
                    is CellState.Sunk -> {
                        damageReport.sunk.add(ArrayList(cellState.positions.map { (x, y) ->
                            Position(x, y)
                        }))
                    }

                    CellState.Hit -> {
                        damageReport.hit.add(Position(x, y))
                    }

                    CellState.Miss -> {
                        damageReport.miss.add(Position(x, y))
                    }
                }
            }
            is Result.Err -> {
